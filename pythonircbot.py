...
class _BotReceiveThread(threading.Thread):
	...
	def __init__(self, bot, verbose=True, *args, **kwargs):
		...
		# Event to fire when a user changes nick
		self._nickchangeEvent = _PyEvent()
	...
	def _nickchange(self, line):
		matchNick = re.compile('^:(.*)!~?.* NICK :(.*)').search(line)
		if matchNick:
			# Nick change
			self._nickchangeEvent.emit(matchNick.group(1), matchNick.group(2))
			return "continue"
	...
	def _kick(self, line):
		matchKick = re.compile('^:.* KICK ([^ ]+) ([^ ]+) :.*$').search(line)
		if matchKick:
			nick = matchKick.group(2)
			channel = matchKick.group(1)
			self._partedEvent.emit(nick, channel)
			return "continue"
...
class Bot(object):
	...
	def connect(self, host, port=6667, verbose=True, sleepTime=0.8, maxItems=4, channels=[]):
		...
		if self._connected:
			...
			self._receiveThread._nickchangeEvent.connect(self._changedNick)
	...
	def _joinedChannel(self, nick, channel):
		if nick == self._nick:
			...
			if 'names' not in self._channels[channel.upper()]:
				self._channels[channel.upper()]['names'] = set()
	...
	def _changedNick(self, oldnick, newnick):
		for ch in self._channels:
			if self._channels[ch]['names'] and (oldnick in self._channels[ch]['names']):
				self._channels[ch]['names'].remove(oldnick)
				self._channels[ch]['names'].add(newnick)
